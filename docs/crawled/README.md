...
- **多图层架构**：行政区域 / 景点 / 美食 / 交通枢纽 / 政府标记，可独立切换；可交互美食点位拥有透明触控热区，并与景点点位共享键盘聚焦高亮
...
- **移动端适配**：响应式布局、收藏页紧凑首屏、极窄屏品牌栏单行、城市比较表容器内横向滚动、手记栏目窄屏横向浏览、带把手与入场动效的底部预览面板、兴趣卡横向滑动、内容页首图 4:3 比例、搜索与路线筛选入口优化，搜索页移动端不自动唤起系统键盘，移动端菜单以实色悬浮面板打开并配合背景遮罩、锁定背景滚动，点击菜单入口后立即收起并将焦点回收到菜单按钮，悬停反馈与桌面导航统一；移动端菜单入口、选择控件、兴趣筛选选中态、地图景点预览动效、路由加载态、公共反馈层安全区、次级行动按钮、安静按钮、文字链接、清除/重置类文字操作、详情返回入口、资料来源入口、搜索清空按钮、筛选面板、路线筛选、行前清除筛选、行前清单、地图图层选中态、地图层级、首页探索、首页内容方法入口、搜索快捷词、内容卡片、阅读入口、资料目录、手记栏目、详情操作、收藏按钮、对比按钮与深色头图按钮聚焦状态统一；桌面与移动导航同步标记当前页面
