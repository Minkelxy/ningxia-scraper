...
import type { XhsNote, Platform } from "../src/schema/note.js";
...
const JOURNAL_FRONTMATTER_FIELDS = [
  ...
  "sourceId", // 本项目 provenance = <platform>:<noteId>
...
export function buildFrontmatter(note: XhsNote): Record<JournalFmKey, unknown> {
  ...
  const slug = `${note.sourcePlatform}-${note.noteId.slice(0, 12)}`;
  ...
  return {
    ...
    slug,
    description: `来自 ${note.sourcePlatform.toUpperCase()} 素材库的候选笔记。作者：${note.authorNickname}，参考发布 ${note.publishedAt ?? "未知"}。必须人工重写 + 二次核对后才能发布。`,
    author: "", // 编辑自己填
    type: journalType,
...
export function buildDraftMd(note: XhsNote): string {
  ...
  lines.push("---");
  lines.push("");
  lines.push(`# 待改写 · 素材来源：[${note.source_id}](${note.sourceUrl})`);
  lines.push("");
...
export function buildTaskCards(notes: XhsNote[]): string {
  const lines: string[] = [];
  const platformLabel = PLATFORM_LABEL[notes[0]?.sourcePlatform ?? "xhs"];
  lines.push(`# 编辑任务卡片（来自 ${platformLabel} 素材库）`);
  lines.push("");
  lines.push(
